...
function NetworkGlobe({ mouse }: NetworkGlobeProps) {
  ...
  const { pointsGeo, linesGeo, outerWireGeo } = useMemo(() => {
    ...
    const wGeo = new THREE.SphereGeometry(RADIUS * 1.02, 24, 24);
  ...
  return (
    <group ref={groupRef}>
      ...
      <lineSegments geometry={linesGeo}>
        <lineBasicMaterial color="#9400ff" transparent opacity={0.45} />
      </lineSegments>
      ...
      <mesh geometry={outerWireGeo}>
        <meshBasicMaterial color="#aed2ff" wireframe transparent opacity={0.07} />
      </mesh>
...
const HeroGlobe = ({ mouse }: HeroGlobeProps) => {
  ...
  return (
    <div ref={containerRef} className="hero-globe-canvas-wrap">
      <Canvas
